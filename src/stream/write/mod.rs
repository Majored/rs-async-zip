...
impl<'b, 'a> AsyncWrite for WriteAdapter<'b, 'a> {
    ...
    fn poll_shutdown(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<std::result::Result<(), tokio::io::Error>> {
...
impl<'a, 'b> ZipStreamWriterGuard<'a, 'b> {
    pub async fn from_writer(
        raw_writer: &'b mut ZipStreamWriter<'a>,
        entry_opts: ZipEntryOptions,
    ) -> Result<ZipStreamWriterGuard<'a, 'b>> {
        ...
        Ok(Self {
            compressed_writer: CompressionWriter::to_writer(
                WriteAdapter {
                    inner: &mut raw_writer.writer,
                    written: 0,
                },
                entry_opts.compression,
            ),
...
impl<'a> ZipStreamWriter<'a> {
    ...
    pub fn new(writer: &'a mut AsyncWriter) -> Self {
        Self {
            writer,
            state: WriterInnerState::default(),
        }
    }
